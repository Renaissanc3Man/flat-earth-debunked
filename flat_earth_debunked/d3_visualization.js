...
//extra vars needed for bar chart setup
var barchartHeight = height/3;
var barchartMargin = { left: 90, top: 30, right: 30, bottom: 30};

var innerWidth = width - (barchartMargin.left + barchartMargin.right);
var innerHeight = barchartHeight - (barchartMargin.top + barchartMargin.bottom);
...
var projection1 = d3.geo.azimuthalEquidistant()
...
//set up scales and axis labels for bar chart
...
var yAxis = d3.svg.axis().scale(yScale).orient("left")
    .ticks(5)          //use approx 5 tick marks
    .outerTickSize(0);
...
function ready(error, world, flightPlans) {
  ...
  //append some divs to draw outlines on the charts
  //call this section of code after generating the countries
  //otherwise, they will be on top of the boarder
  // svg.append("rect")
  //     .attr("class","chartOutline")
  //     .attr("width", width)
  //     .attr("height", height);
  svg1.append("rect")
      .attr("class","chartOutline")
  ...
  xScale.domain(flightPlans.map(function(d){ return d.CITY; }));
  yScale.domain([0, 3000]); //hardcode 3000 because we calculate mph 1 by 1 each step
  //TODO: NEED TO MAKE NEW COLUMNS IN THE DATA WITH CALCULATED VALUES, MAKE ALL BARS FIRST
  ...
  var barTitle = svg2.append("text")
      .attr("class","barTitle")
      .attr("x", width / 2)
      .attr("y", height * 0.05)
      .text("Flight Speed (MPH)");
  ...
  function step() {
    ...
    //globe = 180 degree Longitude corrected
    if(gotoCoordRawData[0] < prevCoordRawData[0]){
      //add distances from 180 degree mark
      var globeDistance = Math.sqrt(Math.pow((180 - Math.abs(prevCoordRawData[0])) + (180-Math.abs(gotoCoordRawData[0])),2) + Math.pow(prevCoordRawData[1] - gotoCoordRawData[1],2));
    } else{
      var globeDistance = Math.sqrt(Math.pow(prevCoordRawData[0] - gotoCoordRawData[0],2) + Math.pow(prevCoordRawData[1] - gotoCoordRawData[1],2));
    };
    ...
    var globeMPH = (globeDistance * 69) / flightTimeRaw;
    ...
    // console.log(globeDistance);
    console.log(globeMPH);
    ...
    country.transition()
        .style("fill", function(d, j) { return j === flightPlanCountryID ? "#27AE60" : "#b8b8b8"; });
    country1.transition()
        .style("fill", function(d, j) { return j === flightPlanCountryID ? "#5D6D7E" : "#b8b8b8"; });
    ...
    var globeBars = g2.selectAll(".globeRect_"+i).data([globeMPH]);
    globeBars.enter().append("rect")
        .attr("class","globeRect_"+i)
        .attr("width", xScale.rangeBand()/2.25);
    globeBars.attr("x",xScale(flightPlans[i].CITY))
        .attr("y",yScale(0))
        .attr("height", 0)
        .attr("fill","#27AE60");
    globeBars.transition()
        .delay(myDelay)
        .duration(flightTime)
        .attr("y",yScale(globeMPH))
        .attr("height", innerHeight - yScale(globeMPH));

    var flatmapBars = g2.selectAll(".flatmapRect_"+i).data([flatmapMPH]);
    flatmapBars.enter().append("rect")
        .attr("class","flatmapRect_"+i)
        .attr("width", xScale.rangeBand()/2.25);
    flatmapBars.attr("x",xScale(flightPlans[i].CITY) + ( xScale.rangeBand() - xScale.rangeBand()/2.25))
        .attr("y",yScale(0))
        .attr("height", 0)
        .attr("fill","#5D6D7E");
    flatmapBars.transition()
        .delay(myDelay)
        .duration(flightTime)
        .attr("y",yScale(flatmapMPH))
        .attr("height", innerHeight - yScale(flatmapMPH));

    console.log("poo");


    //use tween function to make
    //plane get larger then smaller
    plane.transition()
      .delay(myDelay)
      .duration(flightTime)
      .tween("test", function(){
