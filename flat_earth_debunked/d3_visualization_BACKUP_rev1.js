...
function ready(error, world, flightPlans) {
  ...
  function step() {
    ...
    if(i==0){
      prevCoords1 = [projection1([flightPlans[n-1].LONGITUDE, flightPlans[n-1].LATITUDE])[0], projection1([flightPlans[n-1].LONGITUDE, flightPlans[n-1].LATITUDE])[1]];
    } else {
      prevCoords1 = [projection1([flightPlans[i-1].LONGITUDE, flightPlans[i-1].LATITUDE])[0], projection1([flightPlans[i-1].LONGITUDE, flightPlans[i-1].LATITUDE])[1]];
    };
    ...
    plane1.transition()
      ...
      .tween("test",function(){
        return function(t){
          var s = 2.25 * Math.min(Math.sin(Math.PI * t) * 0.7, 0.4) + 0.75;
          var gotoCoords1 = [projection1([flightPlans[i].LONGITUDE, flightPlans[i].LATITUDE])[0], projection1([flightPlans[i].LONGITUDE, flightPlans[i].LATITUDE])[1]];
